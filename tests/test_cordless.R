...
stochastic_data_dose <- lapply(seq_len(length(param_simulations)), function(i) {
  ...
  list(
    body_dect_dose = body_dect_dose,
    brain_dect_dose = brain_dect_dose
  )
})

stochastic_mobiledata_dose_df <- data.frame(
  body_dect_dose = sapply(stochastic_data_dose, \(x) x$body_dect_dose),
  brain_dect_dose = sapply(stochastic_data_dose, \(x) x$brain_dect_dose)
)
...
hist(stochastic_mobiledata_dose_df$body_dect_dose,
     probability = TRUE,
     breaks = 100,
     col = "lightblue",
     border = "white",
...
lines(density(stochastic_mobiledata_dose_df$body_dect_dose),
      col = "red",
      lwd = 2)

abline(v = mean(stochastic_mobiledata_dose_df$body_dect_dose),
       col = "blue",
       lwd = 2,
       lty = 2)

boxplot(stochastic_mobiledata_dose_df$body_dect_dose,
        horizontal = TRUE,
        col = "lightblue",
        main = "Body data dose",
        xlab = "Dose")

stripchart(stochastic_mobiledata_dose_df$body_dect_dose,
           method = "jitter",
           pch = 16,
           cex = 0.5,
           vertical = FALSE,
...
hist(stochastic_mobiledata_dose_df$brain_dect_dose,
     probability = TRUE,
     breaks = 100,
     col = "lightblue",
     border = "white",
...
lines(density(stochastic_mobiledata_dose_df$brain_dect_dose),
      col = "red",
      lwd = 2)

abline(v = mean(stochastic_mobiledata_dose_df$brain_dect_dose),
       col = "blue",
       lwd = 2,
       lty = 2)


boxplot(stochastic_mobiledata_dose_df$brain_dect_dose,
        horizontal = TRUE,
        col = "lightblue",
        main = "Brain data dose",
        xlab = "Dose")

stripchart(stochastic_mobiledata_dose_df$brain_dect_dose,
           method = "jitter",
           pch = 16,
           cex = 0.5,
           vertical = FALSE,
